...
public class FileClient {
	...
	public boolean cropFile(String filename, Map<String, String> params) throws IOException {
		this.isUploadOk = false;
		filename = formatPath(filename);
		HttpURLConnection conn = null;
		try {
	...
	private String sign(HttpURLConnection conn, String uri) {
		String sign = conn.getRequestMethod() + "&" + uri + "&" + conn.getRequestProperty("Date") + "&" + this.contentType + "&" + this.contentMD5 + "&"
				+ this.uid + "&" + this.secretAccessKey;
