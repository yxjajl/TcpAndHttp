...
package com.quartz.examples.example13;
...
public class ClusterExample {
	...
	public void run(boolean inClearJobs, boolean inScheduleJobs) throws Exception {
		...
		if (inScheduleJobs) {
			...
			JobDetail job = newJob(SimpleRecoveryJob.class).withIdentity("job_" + count, schedId) // put
																									// triggers
																									// in
																									// group
																									// named
			...
			job = newJob(SimpleRecoveryJob.class).withIdentity("job_" + count, schedId) // put
																						// triggers
																						// in
																						// group
																						// named
			...
			job = newJob(SimpleRecoveryStatefulJob.class).withIdentity("job_" + count, schedId) // put
																								// triggers
																								// in
																								// group
																								// named
			...
			_log.info(job.getKey() + " will run at: " + trigger.getNextFireTime() + " & repeat: "
