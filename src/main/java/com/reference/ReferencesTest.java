...
public class ReferencesTest {
	...
	public void weakReference() throws InterruptedException {
		...
		WeakReference<Object> weakRerference = new WeakReference<Object>(new VeryBig("weakReference"));
		System.gc();
		Thread.sleep(500);
		/**
	...
	/**
	 * WeakHashMap<br>
	 * 使用 WeakReference 作为 key， 一旦没有指向 key 的强引用, WeakHashMap 在 GC 后将自动删除相关的 entry
	 * 
	 * @throws InterruptedException
	 */
	@Test
	...
	/**
	 * SoftReference<br>
	 * 与 WeakReference 的特性基本一致， 最大的区别在于 SoftReference会尽可能长的保留引用，不会在GC时就回收对象，而是直到 JVM 内存不足时才会被回收(虚拟机保证), 这一特性使得 SoftReference 非常适合缓存应用
	 */
	@Test
	public void softReference() {
		SoftReference<Object> softRerference = new SoftReference<Object>(new VeryBig("softReference"));
	...
	public void phantomReferenceAlwaysNull() {
		...
		PhantomReference<Object> phantomReference = new PhantomReference<Object>(new VeryBig("phantomReferenceAlwaysNull"), q);
	...
	/**
	 * RererenceQueue<br>
	 * 当一个 WeakReference 开始返回 null 时， 它所指向的对象已经准备被回收， 这时可以做一些合适的清理工作. 将一个 ReferenceQueue 传给一个 Reference 的构造函数， 当对象被回收时， 虚拟机会自动将这个weak ref插入到 ReferenceQueue 中， WeakHashMap 就是利用 ReferenceQueue 来清除 key
	 * 已经没有强引用的 entries
	 * 
	 * @throws InterruptedException
	 */
